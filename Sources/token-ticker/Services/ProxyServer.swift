...
final class ProxyServer {
    ...
    private let queue = DispatchQueue(label: "com.token-ticker.proxyServer", qos: .utility)

    /// Serial queue that serializes all log file reads and writes, preventing TOCTOU races.
    private static let logQueue = DispatchQueue(label: "com.token-ticker.proxylog")
    ...
    private init() {
        ...
        let configDir = home.appendingPathComponent(".config/token-ticker")
    ...
    func start(port: UInt16 = 11435) {
        ...
        // Run on logQueue to avoid racing with appendLogEntry
    ...
    private func forwardToUpstream(requestData: Data, clientConn: NWConnection) {
        ...
        // Capture upstream weakly to break the retain cycle
        ...
        // Retain upstream in connections so it isn't held only by the closure chain.
    ...
    nonisolated private static func pipeResponse(from upstream: NWConnection,
        ...
        upstream.receive(minimumIncompleteLength: 1, maximumLength: 65536) { content, _, isComplete, error in
            ...
            if isComplete || error != nil {
                ...
                // Cancel upstream only after the final client send completes so the
    ...
    nonisolated private static func appendLogEntry(model: String,
        ...
        // Serialize all file I/O on logQueue to prevent interleaved writes from concurrent calls.
        logQueue.sync {
